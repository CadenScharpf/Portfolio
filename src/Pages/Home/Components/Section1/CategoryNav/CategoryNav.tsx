...
>((props: ICategoryMenuItemProps, ref) => {
  ...
  const variants = {
    ...
    animate: { width: "100%", opacity:1, transition: { duration: previewDuration/1000, } },
    exit: { left: '100%', transition: { duration: 1.5, type: "spring", bounce: .3  } },
  };
  return (
    <Box
        onClick={() => {
      ...
      >
        ...
        {active && (
          <Box
            ...
            key={props.category.name + (active ? "-active" : "-inactive") + "-nav-item"}
            variants={variants}
            style={{
              position: "absolute",
              left: 0,
